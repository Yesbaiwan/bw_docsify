...
- `siliconflow` 需要添加环境变量 `SILICONFLOW_API_KEY`，值为 siliconflow 的 API Key。
- 五个渠道的名称是：`kilo`、`aiping`、`poe`、`openrouter`、`siliconflow`。
...
export default {
  async fetch(req, env) {
    ...
    if (pathParts.length === 0) {
      return new Response("API Gateway - Available channels: /kilo, /aiping, /poe, /openrouter, /siliconflow", { status: 200 });
    }
